...
def main() -> None:
    ...
    try:
        ...
        tasks_db = TaskDB(
            os.getenv("NOTION_TASK_ID"), 
            NOTION_TOKEN, 
            {"Projects": projects_db, "Sprints": sprints_db}
        )
...
def process_sync_row(row: object, tasks_db: TaskDB, gcal: GoogleCalendarAPI) -> None:
    ...
    if not gcal_event_id:
        ...
        try:
            ...
            # NotionにIDを書き戻す
            tasks_db.update_page(task_id, {
                "GCal_Event_ID": {"rich_text": [{"text": {"content": new_event_id}}]}
            })
        except Exception as e:
    ...
    # GCalの方が新しい -> Notionを更新
        ...
        if gcal_date and gcal_date != work_date:
            tasks_db.update_page(task_id, {
                "作業日": {"date": {"start": gcal_date.isoformat()}}
            })
        else:
